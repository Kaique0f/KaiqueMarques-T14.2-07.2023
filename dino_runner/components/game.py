...
class Game:
    ...
    def draw(self):
        ...
        self.draw_clouds()
        self.player.draw(self.screen)  # Desenha o dinossauro por cima das nuvens
    ...
    def check_collision(self):
        dino_rect = self.player.image.get_rect()
        for cloud in self.clouds:
            cloud_rect = pygame.Rect(cloud[0], cloud[1], CLOUD.get_width(), CLOUD.get_height())
            if dino_rect.colliderect(cloud_rect):
                # O dinossauro colidiu com a nuvem
                self.player.draw(self.screen)  # Desenha o dinossauro novamente para que ele fique na frente da nuvem
                break
